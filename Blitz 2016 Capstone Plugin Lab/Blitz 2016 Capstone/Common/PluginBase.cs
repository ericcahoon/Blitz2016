...
namespace CH.Xrm.Plugins.Common
{
    public abstract class PluginBase : IRegisteredEventsPlugin
    {
        ...
        /// <summary>
        /// Forces the Base class to specify RegistrationEvents.  This will be executed before the derived class's constructor runs! 
        /// </summary>
        ...
        protected virtual bool PreventRecursiveCall(LocalPluginContext context)
        {
            if (context.Event.Message == MessageType.Delete ||
                context.Event.Message == MessageType.Publish ||
                context.Event.Message == MessageType.PublishAll) {
                return false;
            }

            var sharedVariables = context.PluginExecutionContext.SharedVariables;
            var key = String.Format("{0}|{1}|{2}|{3}", context.PluginTypeName, context.Event.MessageName, context.PluginExecutionContext.PrimaryEntityId, context.PluginExecutionContext.Stage);

            if (sharedVariables == null) sharedVariables = new ParameterCollection();  //Ensure the shared variable collection isn't null before checking if the key exists
            if (sharedVariables.ContainsKey(key)) return true;
